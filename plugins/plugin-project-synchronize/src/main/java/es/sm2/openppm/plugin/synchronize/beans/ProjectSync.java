 ...
 * Create Date: 22/03/2015 13:25:35
 */
...
public class ProjectSync {
    ...
    private EmployeeSync employeeByInvestmentManager;
    private String performingorg;
    private String program;
    private String customer;
    private String category;
    private EmployeeSync employeeBySponsor;
    private EmployeeSync employeeByFunctionalManager;
    private EmployeeSync employeeByProjectManager;
    ...
    private ProjectCharterSync projectCharter;
    private StageGateSync stageGate;
    private List<WbsnodeSync> wbsnodes = new ArrayList<WbsnodeSync>();
    private List<FollowupSync> followups = new ArrayList<FollowupSync>();
    private List<StakeholderSync> stakeholders = new ArrayList<StakeholderSync>();
    private List<IncomeSync> incomes = new ArrayList<IncomeSync>();
    private List<AssumptionSync> assumptions = new ArrayList<AssumptionSync>();
    private List<IssuelogSync> issuelogs = new ArrayList<IssuelogSync>();
    private List<RiskSync> risks = new ArrayList<RiskSync>();
    private List<ChargeCostSync> chargeCosts = new ArrayList<ChargeCostSync>();
    private List<WorkingCostSync> workingCosts = new ArrayList<WorkingCostSync>();
    private List<LabelSync> labels = new ArrayList<LabelSync>();

    public ProjectSync() {
		super();
    }

	public ProjectSync(Project project, Session session) {
		super();
		...
		if (project.getEmployeeByInvestmentManager() != null) { setEmployeeByInvestmentManager(new EmployeeSync(project.getEmployeeByInvestmentManager())); }
		...
		if (project.getEmployeeBySponsor() != null) { setEmployeeBySponsor(new EmployeeSync(project.getEmployeeBySponsor())); }
		if (project.getEmployeeByFunctionalManager() != null) { setEmployeeByFunctionalManager(new EmployeeSync(project.getEmployeeByFunctionalManager())); }
		if (project.getEmployeeByProjectManager() != null) { setEmployeeByProjectManager(new EmployeeSync(project.getEmployeeByProjectManager())); }
		if (project.getContracttype() != null) { setContracttype(project.getContracttype().getDescription()); }
		if (project.getStagegate() != null) { setStageGate(new StageGateSync(project.getStagegate())); }
		...
		if (project.getProjectcharters() != null && !project.getProjectcharters().isEmpty()) {
			...
			setProjectCharter(new ProjectCharterSync(projectcharter));
		}
		...
		if (project.getWbsnodes() != null && !project.getWbsnodes().isEmpty()) {
			...
			for (Wbsnode wbsnode: project.getWbsnodes()) {
				...
				getWbsnodes().add(new WbsnodeSync(wbsnode));
			}
		}
		...
		if (project.getStakeholders() != null && !project.getStakeholders().isEmpty()) {
			...
			for (Stakeholder stakeholder: project.getStakeholders()) {
				...
				getStakeholders().add(new StakeholderSync(stakeholder));
			}
		}
		...
		if (project.getProjectfollowups() != null && !project.getProjectfollowups().isEmpty()) {
			...
			for (Projectfollowup followup: project.getProjectfollowups()) {
				...
				getFollowups().add(new FollowupSync(followup));
			}
		}
		...
		if (project.getIncomeses() != null && !project.getIncomeses().isEmpty()) {
			...
			for (Incomes income : project.getIncomeses()) {
				...
				getIncomes().add(new IncomeSync(income));
			}
		}
		...
		if (project.getIssuelogs() != null && !project.getIssuelogs().isEmpty()) {
			...
			for (Issuelog issuelog : project.getIssuelogs()) {
				...
				getIssuelogs().add(new IssuelogSync(issuelog));
			}
		}
		...
		if (project.getAssumptionregisters() != null && !project.getAssumptionregisters().isEmpty()) {
			...
			for (Assumptionregister assumptionregister : project.getAssumptionregisters()) {
				...
				getAssumptions().add(new AssumptionSync(assumptionregister));
			}
		}
		...
		if (project.getRiskregisters() != null && !project.getRiskregisters().isEmpty()) {
			...
			for (Riskregister riskregister : project.getRiskregisters()) {
				...
				getRisks().add(new RiskSync(riskregister));
			}
		}
		...
		if (project.getChargescostses() != null && !project.getChargescostses().isEmpty()) {
			...
			for (Chargescosts chargecost : project.getChargescostses()) {
				...
				getChargeCosts().add(new ChargeCostSync(chargecost));
			}
		}
		...
		if (project.getWorkingcostses() != null && !project.getWorkingcostses().isEmpty()) {
			...
			for (Workingcosts workingcost : project.getWorkingcostses()) {
				...
				getWorkingCosts().add(new WorkingCostSync(workingcost));
			}
		}
		...
		if (!projects.isEmpty()) {
			...
			if (labels != null && !labels.isEmpty()) {
				...
				for (Label label : labels) {
					getLabels().add(new LabelSync(label));
				}
			}
		}
	}
	 ...
	 */
	public EmployeeSync getEmployeeByInvestmentManager() {
	 ...
	 */
	public void setEmployeeByInvestmentManager(
			EmployeeSync employeeByInvestmentManager) {
	 ...
	 */
	public EmployeeSync getEmployeeBySponsor() {
	 ...
	 */
	public void setEmployeeBySponsor(EmployeeSync employeeBySponsor) {
	 ...
	 */
	public EmployeeSync getEmployeeByFunctionalManager() {
	...
	public void setEmployeeByFunctionalManager(
			EmployeeSync employeeByFunctionalManager) {
	 ...
	 */
	public EmployeeSync getEmployeeByProjectManager() {
	 ...
	 */
	public void setEmployeeByProjectManager(EmployeeSync employeeByProjectManager) {
	 ...
	 */
	public ProjectCharterSync getProjectCharter() {
	 ...
	 */
	public void setProjectCharter(ProjectCharterSync projectCharter) {
	 ...
	 * @return the wbsnodes
	 */
	public List<WbsnodeSync> getWbsnodes() {
		return wbsnodes;
	}
	 ...
	 */
	public void setWbsnodes(List<WbsnodeSync> wbsnodes) {
	 ...
	 */
	public List<FollowupSync> getFollowups() {
		return followups;
	}
	 ...
	 */
	public void setFollowups(List<FollowupSync> followups) {
	 ...
	 */
	public List<StakeholderSync> getStakeholders() {
		return stakeholders;
	}
	 ...
	 */
	public void setStakeholders(List<StakeholderSync> stakeholders) {
	 ...
	 * @return the incomes
	 */
	public List<IncomeSync> getIncomes() {
		return incomes;
	}
	 ...
	 */
	public void setIncomes(List<IncomeSync> incomes) {
	 ...
	 */
	public List<AssumptionSync> getAssumptions() {
		return assumptions;
	}
	 ...
	 */
	public void setAssumptions(List<AssumptionSync> assumptions) {
	 ...
	 */
	public List<IssuelogSync> getIssuelogs() {
		return issuelogs;
	}
	 ...
	 */
	public void setIssuelogs(List<IssuelogSync> issuelogs) {
	 ...
	 */
	public void setRisks(List<RiskSync> risks) {
		this.risks = risks;
	}
	 ...
	 */
	public List<ChargeCostSync> getChargeCosts() {
		return chargeCosts;
	}
	 ...
	 */
	public void setChargeCosts(List<ChargeCostSync> chargeCosts) {
	 ...
	 */
	public List<WorkingCostSync> getWorkingCosts() {
		return workingCosts;
	}
	 ...
	 */
	public void setWorkingCosts(List<WorkingCostSync> workingCosts) {
	 ...
	 * @return the labels
	 */
	public List<LabelSync> getLabels() {
		return labels;
	}
	 ...
	 */
	public void setLabels(List<LabelSync> labels) {
		this.labels = labels;
	}
	 ...
	 */
	public StageGateSync getStageGate() {
		return stageGate;
	}
	 ...
	 */
	public void setStageGate(StageGateSync stageGate) {
