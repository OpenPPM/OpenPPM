...
<%--
  ...
  ~
  ~ Module: front
  ~ File: reports_project_resource.ajax.jsp
				...
				<thead>
					<tr>
						<c:forEach var="recurso" items="${recursos}">
							<th><nobr>${recurso }</nobr></th>
						</c:forEach>
					</tr>
				</thead>
				<tbody>
					<c:forEach var="proyecto" items="${data}" begin="1">
						<tr>
							<c:forEach var="recurso" items="${proyecto}" begin="1">
								<td class="center">
									${tl:toCurrency(recurso)}
								</td>
							</c:forEach>
						</tr>
					</c:forEach>
