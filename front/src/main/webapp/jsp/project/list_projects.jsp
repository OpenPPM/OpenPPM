...
<c:choose>
	<c:when test="<%=SettingUtil.getBoolean(settings, Settings.SETTING_DISABLE_PROJECT, Settings.DEFAULT_DISABLE_PROJECT)%>">
		<fmt:message key="disable" var="msgTitleDeleteProject"/>
		<fmt:message key="msg.project.disable.confirm" var="msgConfirmDeleteProject"/>
		<fmt:message key="msg.project.disable" var="msgTitleConfirmDeleteProject"/>
	</c:when>
	<c:otherwise>
		<fmt:message key="delete" var="msgTitleDeleteProject"/>
		<fmt:message key="msg.project.disable.confirm" var="msgConfirmDeleteProject"/>
...
<c:if test="<%=SecurityUtil.isUserInRole(request, Constants.ROLE_PMO) %>">
	<c:set var="deleteProject">&nbsp;&nbsp;&nbsp;<img onclick="deleteProject('+aData[0]+');" title="${msgTitleDeleteProject }" class="link" src="images/delete.jpg"></c:set>
</c:if>


<c:set var="projectScript">
	<c:if test="<%=Settings.REPORT_EXECUTIVE%>">
	{
		"sExtends" : "text",
		"sButtonText": '<fmt:message key="report_executive" />',
		"fnClick": function ( nButton, oConfig, oFlash ) { reportProjectsExecutive(); }
	},
	</c:if>
	{
		"sExtends" : "text",
	...
	<c:if test="<%=(SettingUtil.getString(settings, Settings.SETTING_PROJECT_EXECUTIVE_CLASSNAME, Settings.DEFAULT_PROJECT_EXECUTIVE_CLASSNAME) != null)%>">
	{
		"sExtends" : "text",
		"sButtonText": '<fmt:message key="executive_report" />',
		"fnClick": function ( nButton, oConfig, oFlash ) { generateExecutiveReport(); }
	},
	</c:if>
	{
...
function generateExecutiveReport() {
	...
	f.action = '<%=ProjectServlet.REFERENCE%>';
	f.accion.value = "<%=ProjectServlet.GENERATE_EXECUTIVE_REPORT%>";
	f.ids.value = dataInfoTable.ids;
	f.submit();
}
</script>
...
<c:if test="<%=Settings.REPORT_EXECUTIVE%>">
<script language="javascript" type="text/javascript" >
function reportProjectsExecutive() {
	
	var f = document.forms["frm_projects"];
	applyInfoSort();
	
	f.action = "<%= FollowProjectsServlet.REFERENCE %>";
	f.accion.value = "<%= FollowProjectsServlet.REPORT_PROJECTS_EXECUTIVE %>";
	f.ids.value = dataInfoTable.ids;
...
var projectsTable,
	...
	loadRiskReport = false,
	loadGantt = false,
	loadResourceCR = false,
	loadMonthCR = false,
	loadActivityResourceCR = false,
	loadActivityMonthCR = false,
	loadKPIs = false;
...
// Redirect to selection tab
function viewProjectTab(id, tab) {
	...
	if (tab == '<%=Constants.TAB_INITIATION%>') {
		f.action = "<%=ProjectInitServlet.REFERENCE%>";
	}
	else if (tab == '<%=Constants.TAB_PLAN%>') {
		f.action = "<%=ProjectPlanServlet.REFERENCE%>";
	}
	else if (tab == '<%=Constants.TAB_CONTROL%>') {
		f.action = "<%=ProjectControlServlet.REFERENCE%>";
	}
	else if (tab == '<%=Constants.TAB_RISK%>') {
		f.action = "<%=ProjectRiskServlet.REFERENCE%>";
	}
	else if (tab == '<%=Constants.TAB_PROCURAMENT%>') {
		f.action = "<%=ProjectProcurementServlet.REFERENCE%>";
	}
	else if (tab == '<%=Constants.TAB_CLOSURE%>') {
		f.action = "<%=ProjectClosureServlet.REFERENCE%>";
	}
    else if (tab == '<%= Constants.TAB_LEARNED_LESSON %>') {
        f.action = "<%= ProjectLearnedLessonServlet.REFERENCE %>";
    }
...
function reportProjects() {
	...
	f.<%= Milestones.MILESTONECATEGORY %>.value = $("#milestonecategory").val();
...
function loadProjects() {
	...
	projectAjax.call(aoData,function (data) {
		...
		if (loadRiskReport) { loadRiskReport = false; infoRiskReport(); }
		if (loadGantt) { loadGantt = false; showChartGantt(); }
		<%if (SettingUtil.getBoolean(settings, Settings.SETTING_PERFORMANCE_REPORTS, Settings.DEFAULT_PERFORMANCE_REPORTS)) {%>
		if (loadResourceCR) { loadResourceCR = false; projectResource.show(); }
		if (loadMonthCR) { loadMonthCR = false; projectMonth.show(); }
		if (loadActivityResourceCR) { loadActivityResourceCR = false; projectActivityResource.show(); }
		if (loadActivityMonthCR) { loadActivityMonthCR = false; projectActivityMonth.show(); }
		<%}%>
...
<form id="frm_projects" name="frm_projects" method="post" onsubmit="return false;">
	...
	<input type="hidden" name="<%= Milestones.MILESTONECATEGORY %>" />
		...
		<table id="tb_projects" class="tabledata" cellspacing="1" width="100%">
			<thead>
				<tr>
					...
					<th><fmt:message key="setting.project.column.actual_cost" /></th>
...
<%-- RISK REPORT --%>
<jsp:include page="common/risk_report.inc.jsp" flush="true"/>

<%-- MILESTONES TABLE --%>
<jsp:include page="common/milestones.jsp" flush="true"></jsp:include>
...
<%-- KPIS AGGREGATE --%>
<jsp:include page="common/aggregate_kpis.jsp" flush="true"/>

<c:if test="<%=SettingUtil.getBoolean(settings, Settings.SETTING_PERFORMANCE_REPORTS, Settings.DEFAULT_PERFORMANCE_REPORTS)%>">
	<%-- PERFORMANCE REPORTS --%>
	<jsp:include page="performance_reports/reports.inc.jsp" flush="true"/>
</c:if>