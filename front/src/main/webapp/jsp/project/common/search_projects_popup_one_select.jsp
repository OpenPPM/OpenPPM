...
<%--
  ...
  ~
  ~ Module: front
  ~ File: search_projects_popup_one_select.jsp
		...
		for (var i = 0; i < data.length; i++) {
			...
			if ((data[i].<%=Project.IDPROJECT%>+'') != '${project.idProject}') {
				searchProjectsTable.fnAddData([
						...
						'<img class="icono"  title="<fmt:message key="add"/>" src="images/AddContact.png" onclick="addProjectSelected(this);" />'
					]);
			}
		}
...
function addProjectSelected(element) {
	
	var project = searchProjectsTable.fnGetData( element.parentNode.parentNode );
	
	$(idSelectForAdd).val(project[0]);
	$(idSelectForAdd+"Name").val(project[1]);
...
function deleteProject() {
	...
	$(idSelectForAdd).val("-1");
	$(idSelectForAdd+"Name").val("");
...
<div id="search-projects-popup" class="popup">	
		...
		<form id="frm_search_project" name="frm_search_project" onsubmit="searchProject; return false;">
			...
			<table id="tb_searchproject" class="tabledata" cellspacing="1" width="100%">
				<thead>
					<tr>
						<th>&nbsp;</th>
						<th width="50%"><fmt:message key="project" /></th>
	...
	<div class="popButtons">
		<a href="javascript:deleteProject();" class="boton"><fmt:message key="delete" /></a>
