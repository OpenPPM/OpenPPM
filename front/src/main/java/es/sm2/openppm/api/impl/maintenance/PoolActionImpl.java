 ...
 *
 * Module: front
 * File: PoolActionImpl.java
 * Create User: javier.hernandez
 * Create Date: 25/09/2015 12:45:09
 */

package es.sm2.openppm.api.impl.maintenance;

import es.sm2.openppm.api.converter.maintenance.PoolDTOConverter;
 ...
 */
public class PoolActionImpl implements MaintenanceAction {
    ...
    public Response findAll(Company company, Employee user) {
        ...
        try {
            // Find data
            ResourcepoolLogic logic = new ResourcepoolLogic();
            List<Resourcepool> items;

            if (Resourceprofiles.Profile.RESOURCE_MANAGER.getID() == user.getResourceprofiles().getIdProfile()) {

                items = logic.findByResourceManager(user);
            }
            else {
                items = logic.findByRelation(Contracttype.COMPANY, company, Resourcepool.NAME, Order.ASC);
            }

            // Convert to DTO and Send data
            PoolDTOConverter converter = new PoolDTOConverter();
